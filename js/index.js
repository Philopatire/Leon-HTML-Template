// Menu
{
	let outerBtn = document.querySelector('nav .container .icon'),
		innerBtn = document.querySelector('.menu .close'),
...
// Portfolio
{
	...
	let picToAdd = {
		one: {
			...
			title: 'Project Here',
			pargraph: `My creative ability is very difficult to measure because 
	...
	function addPort(url, title, pargraph) {
		...
		add.innerHTML = `<img src= ${url} alt="Image-${++picNums}" />
  <div class="caption">
  <h4>${title}</h4>
  <p>
    ${pargraph}
